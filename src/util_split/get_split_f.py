...
def process_and_save(data, output_dir, groups):
    ...
    """
    # 创建类别到数据的映射
    category_to_data = {i: [] for i in range(1,240)}
    for entry in data:
        parts = entry.split(',')
        if len(parts) > 1:  # 确保数据中至少有三个字段
            try:
                category = int(parts[1])  # 只提取第二个逗号之后的第一个数字
...
def main():
    # 输入文件路径列表（3个数据集）
    input_files = ["../Jsonf/fcv_train.txt", "../Jsonf/fcv_test.txt", "../Jsonf/fcv_val.txt"]
    output_dirs = ["../splitf", "../split_testf", "../split_valf"]

    # 类别列表（1-239）
    categories = list(range(1, 240))

    # 随机打乱类别
    random.shuffle(categories)

    # 将 239 类分为 10 组，每组 24 或 23 类
    group_size = [24] * 9 + [23]   # 9组24类 + 1组23类 = 239
    groups = []
    start = 0
    for size in group_size:
        groups.append(categories[start:start+size])
        start += size

...
def process_and_save(data, output_dir, groups):
    ...
    """
    # 创建类别到数据的映射
    category_to_data = {i: [] for i in range(1,102)}
    for entry in data:
        parts = entry.split(',')
...
def main():
    # 输入文件路径列表（3个数据集）
    input_files = ["../Jsonu/train.txt", "../Jsonu/test.txt", "../Jsonu/val.txt"]  # 数据集文件路径
    output_dirs = ["../splitu", "../splitu_test", "../splitu_val"]  # 每个数据集的输出目录

    # 类别列表（0-199）
    categories = list(range(1,102))
